...
const deleteApp = async (req, res) => {
  // delete the app and all of its data.
...
module.exports = (app) => {
   ...
   * @param {*} res
   */
  // NEW API TO GET ALL APPS
  app.get('/app/list', async (req, res) => {
    const body = req.body || {};
    const { secureKey: parsedSecureKey } = body;
    if (config.secureKey === parsedSecureKey) {
      try {
        const appList = await appService.getAppList();
  ...
  app.put('/app/:appId/:tableName', isAppDisabled, async (req, res) => {
    ...
    try {
      ...
      const isAuthorized = await appService.isClientAuthorized(appId, appKey, authorizationLevel, table);
      if (isAuthorized) {
        const updatedTable = await appService.upsertTable(appId, tableName, body.data.columns, body.data);
  ...
  app.post('/export/:appId/:tableName', async (req, res) => {
    ...
    try {
      const isMasterKey = await appService.isMasterKey(appId, appKey);
      const data = await otherService.exportTable(appId, tableName, expType, isMasterKey, accessList);
      if (expType === 'json') {
  ...
  app.post('/import/:appId', async (req, res) => {
    const {
      key: appKey, tableName, fileName, fileId,
    } = req.body;
